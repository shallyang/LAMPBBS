...
if(!empty($_GET)){
    ...
    //获取总行数
    $sql = "select user.name,posts.* from user,posts where posts.recycle=1 $banKuaiMing and uid=user.id and posts.title like '%{$seach}%' ";
    $result = mysqli_query($link, $sql);
    $count = mysqli_affected_rows($link) ? mysqli_affected_rows($link) : 0;

    $sql = "select user.name,posts.* from user,posts where posts.recycle=1 $banKuaiMing and uid=user.id and posts.title like '%{$seach}%' limit {$start},5";
}else{
    //获取总行数
    $sql = "select user.name,posts.* from user,posts where posts.recycle=1 $banKuaiMing and uid=user.id";
    $result = mysqli_query($link, $sql);
    $count = mysqli_affected_rows($link) ? mysqli_affected_rows($link) : 0;

    //若果是直接进入,无GET
    $sql = "select user.name,posts.* from user,posts where posts.recycle=1 $banKuaiMing and uid=user.id limit {$start},5";
...
<table width="99%" border="0" cellspacing="0" cellpadding="0" id="searchmain">
    ...
    <table width="100%" border="0" cellspacing="0" cellpadding="0" id="main-tab">
      ...
      <tr onMouseOut="this.style.backgroundColor='#ffffff'" onMouseOver="this.style.backgroundColor='#edf5ff'">
        ...
        <td align="center" valign="middle" class="borderbottom">
            ...
            <a href="#" target="mainFrame" onFocus="this.blur()" class="add">禁止回复</a>
        </td>
        <td align="center" valign="middle" class="borderbottom">
            <?php
            ...
            <a href="#" target="mainFrame" onFocus="this.blur()" class="add">管理回复(<?php echo $tieZiCount; ?>)</a><span class="gray">&nbsp;|&nbsp;</span>
            <?php
            }else{
            ?>
  ...
  <tr>
    <td align="left" valign="top" class="fenye">
        ...
        <a href="post_rec.php?start=0&seach=<?php if(isset($seach)){echo $seach;} ?>&bankuaiid=<?php if(isset($banKuaiFenQu)){echo $banKuaiFenQu;} ?>" target="mainFrame" onFocus="this.blur()">首页</a>&nbsp;&nbsp;
        <a href="post_rec.php?start=<?php if($start-5 < 0){echo 0;}else{echo $start - 5;} ?>&seach=<?php if(isset($seach)){echo $seach;} ?>&bankuaiid=<?php if(isset($banKuaiFenQu)){echo $banKuaiFenQu;} ?>" target="mainFrame" onFocus="this.blur()">上一页</a>&nbsp;&nbsp;
        <a href="post_rec.php?start=<?php if($start < $lastPage){echo $start + 5;}else{echo $lastPage;}?>&seach=<?php if(isset($seach)){echo $seach;} ?>&bankuaiid=<?php if(isset($banKuaiFenQu)){echo $banKuaiFenQu;} ?>" target="mainFrame" onFocus="this.blur()">下一页</a>&nbsp;&nbsp;
        <a href="post_rec.php?start=<?php echo $lastPage; ?>&seach=<?php if(isset($seach)){echo $seach;} ?>&bankuaiid=<?php if(isset($banKuaiFenQu)){echo $banKuaiFenQu;} ?> " target="mainFrame" onFocus="this.blur()">尾页</a>
    </td>
  </tr>
</table>
<?php
